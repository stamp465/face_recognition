...
usbMsgLen_t usbFunctionSetup(uint8_t data[8])
{
    ...
    else if (rq->bRequest == RQ_SET_START)
    {
        ...
        if (if_face == 1 && set_start == 0)
        { // when see face and before status is close door
            set_start = 1;
            state = 1;
        }
    ...
    else if (rq->bRequest == RQ_GET_PW)
    {
        ...
        { // when submit and not waiting for result and all is not zero
        ...
        { // when submit and all is zero --- ignore waiting
            submit = false;
            usbMsgPtr = (uint8_t *)&password_notsubmit;
            print_password();
        }
        else
        { // when not submit ( python call all time )
            usbMsgPtr = (uint8_t *)&password_notsubmit;
        }
...
void loop()
{
    ...
    if (set_start == 1)
    {
        ...
        if (button_sw)
        {
            if (!digitalRead(PIN_PB0) && size_now < pass_length)
            { // if 3 pressed and password not full of screen
                password[size_now] = 3;
                size_now++;
                while (!digitalRead(PIN_PB0))
                    usbPoll();
            }
            else if (!digitalRead(PIN_PB1) && size_now < pass_length)
            { // if 5 pressed and password not full of screen
                password[size_now] = 5;
                size_now++;
                while (!digitalRead(PIN_PB1))
                    usbPoll();
            }
            else if (!digitalRead(PIN_PB2) && size_now < pass_length)
            { // if 7 pressed and password not full of screen
                password[size_now] = 7;
                size_now++;
                while (!digitalRead(PIN_PB2))
                    usbPoll();
            }
            else if (!digitalRead(PIN_PB3) && size_now < pass_length)
            { // if 9 pressed and password not full of screen
                password[size_now] = 9;
                size_now++;
            ...
            else if (!digitalRead(PIN_PB5) && size_now > 0)
            { // backspace if have password in screen
                password[size_now - 1] = 0;
                size_now--;
                while (!digitalRead(PIN_PB5))
                    usbPoll();
            }
            else if (!digitalRead(PIN_PB4))
            { // if ENTER
                submit = true;
